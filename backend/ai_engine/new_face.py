...
DETECTION_SCALE = 0.75  # Downscale frame for face detection — 0.75 preserves detail for longer range

# Accuracy settings
UPSAMPLE_TIMES = 1       # Upsample before detection — finds smaller/distant faces
NUM_JITTERS = 1          # Re-samples face for embedding — higher = more accurate, slower (1 is good balance)
REGISTRATION_FRAMES = 5  # Number of frames to average for registration embedding
...
REGISTERED_IMAGES_DIR = settings.FACE_REGISTRATIONS_DIR
MISMATCH_CAPTURES_DIR = settings.FACE_MISMATCH_DIR
MULTI_PERSON_CAPTURES_DIR = settings.MULTI_PERSON_DIR
...
class FaceMatchPipeline:
    ...
    def detect_all_faces(self, frame):
        ...
        """
        try:
            # Downscale for faster detection while preserving enough detail
            ...
            # Apply CLAHE to improve detection under poor/variable lighting
            ...
            # upsample=1 detects smaller/more distant faces
            faces = self.face_detector(rgb_img, UPSAMPLE_TIMES)
            num_faces = len(faces)
    ...
    def _get_face_embedding(self, image_path_or_frame):
        """Extract face embedding from first detected face with preprocessing"""
        try:
            ...
            face_embedding = self.face_recognizer.compute_face_descriptor(rgb_img, shape, NUM_JITTERS)
...
class StudentRegistration:
    ...
    def capture_registration_photo(self, candidate_id, name=None, email=None):
        ...
        if captured and captured_frame is not None:
            ...
            # Generate averaged face embedding from multiple captures for robustness
            print("\nCapturing additional frames for a more accurate embedding — hold still...")
            cap2 = cv2.VideoCapture(0)
            embeddings = []
            attempts = 0
            max_attempts = REGISTRATION_FRAMES * 3

            while len(embeddings) < REGISTRATION_FRAMES and attempts < max_attempts:
                ret2, extra_frame = cap2.read()
                if ret2:
                    emb = self._get_face_embedding(extra_frame)
                    if emb is not None:
                        embeddings.append(emb)
                        print(f"  Frame {len(embeddings)}/{REGISTRATION_FRAMES} captured")
                    time.sleep(0.2)
                attempts += 1
            ...
            if not embeddings:
                emb = self._get_face_embedding(captured_frame)
                if emb is not None:
                    embeddings = [emb]

            if embeddings:
                embedding = np.mean(embeddings, axis=0)
                logger.info(f"Registration embedding averaged from {len(embeddings)} frames")
                ...
                print(f"Embedding averaged from {len(embeddings)} frames")
    ...
    def _get_face_embedding(self, frame):
        """Extract face embedding from frame with preprocessing"""
        try:
            # Apply CLAHE for lighting normalisation
            ...
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            faces = self.face_detector(rgb_frame, UPSAMPLE_TIMES)
            ...
            embedding = self.face_recognizer.compute_face_descriptor(rgb_frame, shape, NUM_JITTERS)
